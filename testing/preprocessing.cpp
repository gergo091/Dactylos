...
Robo::Preprocessing::Preprocessing(const cv::Mat & image){
	this->image = image;
...
Robo::Preprocessing::Preprocessing(const cv::Mat & image, int horizontal_resolution, int vertical_resolution){
	this->image = image;
...
cv::Mat Robo::Preprocessing::Segmentation(cv::Mat & image){
...
cv::Mat Robo::Preprocessing::OrientationMap(cv::Mat & image){
	...
	Set_sizeofBlockOrientation(15);
	int block = Get_sizeofBlockOrientation();
...
cv::Mat Robo::Preprocessing::DrawOrientationMap(cv::Mat & image){
...
cv::Mat Robo::Preprocessing::GaborFilter(cv::Mat & image){
	cv::Point anchor(-1, -1);
	cv::Mat kernel;
		...
		for (int j = (sizeofBlockGabor - 1) / 2; j < orientationMap.cols - (sizeofBlockGabor - 1) / 2; j++){
			...
			for (int k = i - (sizeofBlockGabor - 1) / 2; k < i + sizeofBlockGabor - (sizeofBlockGabor - 1) / 2; k++){
				for (int l = j - (sizeofBlockGabor - 1) / 2; l < j + sizeofBlockGabor - (sizeofBlockGabor - 1) / 2; l++){
					sucet += (image.at<uchar>(k, l)/255) * kernel.at<double>(u, v);
					v++;
				}
				v = 0;
				u++;
...
cv::Mat Robo::Preprocessing::GaborFilter(cv::Mat & image, int block_size, int t_sigma, float t_lambda, float t_gamma){
	cv::Point anchor(-1, -1);
	cv::Mat kernel;
	cv::Mat tempOrientationMap = orientationMap.clone();
	gabor = cv::Mat::zeros(image.rows, image.cols, image.type());
	...
	int ddepth = -1, u = 0, v = 0, count = 0;
	int kernel_size = 0;
	int sizeofBlockGabor = block_size;
	double psi = 0;
	...
	for (int i = (sizeofBlockGabor - 1) / 2; i < orientationMap.rows - (sizeofBlockGabor - 1) / 2; i++){
		for (int j = (sizeofBlockGabor - 1) / 2; j < orientationMap.cols - (sizeofBlockGabor - 1) / 2; j++){
			...
			kernel = cv::getGaborKernel(dim, t_sigma, tempOrientationMap.at<double>(i, j), t_lambda, t_gamma, psi, CV_64F);
...
cv::Mat Robo::Preprocessing::Binarization(cv::Mat & image){
	threshold(image, binarization, 100, 255, CV_THRESH_BINARY);

	return binarization;
}

cv::Mat Robo::Preprocessing::ThinningImage(cv::Mat & image){
...
int Robo::Preprocessing::supplement_cols(int W, int W_col){
	int CW;
...
int Robo::Preprocessing::supplement_rows(int W, int W_row){
	int CW;
...
void Robo::Preprocessing::wiener_to_mask(int block_size_x, int block_size_y){
	cv::Mat kernel;
...
void Robo::Preprocessing::GuoHall(cv::Mat & image, int iteration){
...
void Robo::Preprocessing::ChangeBlacToWhite(cv::Mat & image){
	for (int i = 0; i < image.rows; i++){
		for (int j = 0; j < image.cols; j++){
...
void Robo::Preprocessing::ChangeWhiteToBlack(cv::Mat & image){
	for (int i = 0; i < image.rows; i++){
		for (int j = 0; j < image.cols; j++){
			if (image.at<uchar>(i, j) == 255){
				image.at<uchar>(i, j) = 0;
